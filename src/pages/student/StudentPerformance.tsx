    ...
    <div className="space-y-8 max-w-6xl mx-auto pb-12 select-none">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <div className="p-5 rounded-3xl bg-white border border-purple-100 space-y-1 shadow-[0_4px_20px_-2px_rgba(106,27,154,0.05)]">
          ...
          <div className="font-display font-black text-2xl text-[#6A1B9A]">{avgScore}%</div>
        </div>

        <div className="p-5 rounded-3xl bg-white border border-purple-100 space-y-1 shadow-[0_4px_20px_-2px_rgba(106,27,154,0.05)]">
          <span className="text-[10px] uppercase font-bold text-slate-500">Highest Score</span>
          <div className="font-display font-black text-2xl text-emerald-600">{highestScore}%</div>
        </div>

        <div className="p-5 rounded-3xl bg-white border border-purple-100 space-y-1 shadow-[0_4px_20px_-2px_rgba(106,27,154,0.05)]">
          <span className="text-[10px] uppercase font-bold text-slate-500">Lowest Score</span>
          <div className="font-display font-black text-2xl text-slate-600">{lowestScore}%</div>
        </div>

        <div className="p-5 rounded-3xl bg-white border border-purple-100 space-y-1 shadow-[0_4px_20px_-2px_rgba(106,27,154,0.05)]">
          <span className="text-[10px] uppercase font-bold text-slate-500">Tests Cleared</span>
          <div className="font-display font-black text-2xl text-cyan-700">{passedTests} / {totalTests}</div>
        </div>

        <div className="p-5 rounded-3xl bg-white border border-purple-100 space-y-1 col-span-2 lg:col-span-1 shadow-[0_4px_20px_-2px_rgba(106,27,154,0.05)]">
          <span className="text-[10px] uppercase font-bold text-slate-500">Pass Rate</span>
          <div className="font-display font-black text-2xl text-purple-700">{passPercentage}%</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="h-64 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={scoreTrendData}>
                ...
                <Tooltip
                  contentStyle={{ backgroundColor: '#FFFFFF', borderColor: '#E9D6F7', borderRadius: '12px', fontSize: '12px', color: '#1E1035' }}
                />
        ...
        <div className="p-6 rounded-3xl bg-white border border-purple-100 space-y-6 shadow-[0_4px_20px_-2px_rgba(106,27,154,0.05)]">
          <div>
            ...
            <p className="text-xs text-slate-500">Mastery comparison across topics</p>
          </div>
      ...
      <div className="p-6 rounded-3xl bg-white border border-purple-100 space-y-4 shadow-[0_4px_20px_-2px_rgba(106,27,154,0.05)]">
        <h3 className="font-display font-bold text-lg text-slate-900">Assessment Attempt Log</h3>
            ...
            <tbody className="divide-y divide-purple-50">
              {testAttempts.map((att: any) => (
                <tr key={att.id} className="hover:bg-purple-50/50 transition-colors">
                  <td className="p-3 font-semibold text-slate-800">{att.test_title || att.title}</td>
                  <td className="p-3 text-[#6A1B9A] font-bold">{att.subject}</td>
                  <td className="p-3 font-mono text-slate-700">{att.score} / {att.total_marks}</td>
                  <td className="p-3 font-bold text-slate-800">{att.percentage}%</td>
                  <td className="p-3">
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${
                      att.passed ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-rose-100 text-rose-800 border border-rose-200'
                    }`}>
                      {att.passed ? 'PASSED' : 'NEEDS IMPROVEMENT'}
                    </span>
                  </td>
                  <td className="p-3 text-slate-500">{att.submitted_at}</td>
                </tr>
              ))}
            </tbody>
          </table>
